      ...
      {slice.map((p, i) => {
        ...
        return (
          ...
          >
            ...
            <img src={imgUrl(p.image)} alt={p.name} onError={(e) => { e.currentTarget.src = "/assets/offerton.png"; e.currentTarget.classList.add("is-fallback"); }} />
...
function HomePageBody() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/business/nearby?lat=${nearbyLat}&lng=${nearbyLng}&radius=${effectiveRadius}`).then((r) => { if (!r.ok) throw new Error(); return r.json(); }).then((data: NearbyHomeBusiness[]) => { setNearbyBizList(Array.isArray(data) ? data : []); lastFetchedNearbyCoordsRef.current = { lat: nearbyLat, lng: nearbyLng }; }).catch(() => setNearbyBizError("No pudimos cargar los negocios cercanos.")).finally(() => setNearbyBizLoading(false));
  ...
  useEffect(() => { setLoading(true); const featuredExtra: Record<string, string> = { limit: "60" }; if (activeCategory) featuredExtra.category = activeCategory; if (searchParam) featuredExtra.search = searchParam; const params = buildLocationParams(featuredExtra); fetch(`${API}/products/featured?${params}`).then((r) => r.json()).then(async (data) => { let featured: Product[] = data.products || []; if (activeCategory) featured = featured.filter((p) => p.category === activeCategory); const randomExtra: Record<string, string> = { limit: "40" }; if (activeCategory) randomExtra.category = activeCategory; if (searchParam) randomExtra.search = searchParam; if (featured.length === 0) { const r2 = await fetch(`${API}/products/random?${buildLocationParams(randomExtra)}`); const d2 = await r2.json(); setAllProducts(d2.products || []); } else { const excludeIds = featured.map((p) => p._id); if (excludeIds.length) randomExtra.excludeIds = JSON.stringify(excludeIds); const r2 = await fetch(`${API}/products/random?${buildLocationParams(randomExtra)}`); const d2 = await r2.json(); setAllProducts([...featured, ...(d2.products || [])]); } }).catch(async () => { try { const randomExtra: Record<string, string> = { limit: "40" }; if (activeCategory) randomExtra.category = activeCategory; if (searchParam) randomExtra.search = searchParam; const r2 = await fetch(`${API}/products/random?${buildLocationParams(randomExtra)}`); const d2 = await r2.json(); setAllProducts(d2.products || []); } catch { setAllProducts([]); } }).finally(() => setLoading(false)); }, [currentUserId, userHasLoc, userRadius, activeCategory, searchParam]);
  ...
  return (
    <article className={`product-card ${isFeatured ? "product-card--featured" : ""} ${isFlash ? "product-card--flash" : ""}`}>
      <div className="product-image-wrap">
        <img src={imgUrl(product.image)} alt={product.name} loading="lazy" onError={(e) => { e.currentTarget.src = "/assets/offerton.png"; e.currentTarget.classList.add("is-fallback"); }} />
...
export default function HomeContent() {
  return <Suspense fallback={<MainLayout><div style={{ padding: "4rem", textAlign: "center", color: "var(--text-muted)" }}><Clock size={32} style={{ opacity: 0.3, display: "block", margin: "0 auto 1rem" }} /><p>Cargando...</p></div></MainLayout>}><HomePageBody /></Suspense>;
}
