...
const FALLBACK_APK = "https://github.com/contactooffertas/FrontEndAmbos/releases/download/android-v3.6.0/Rosario-Market-3.6.apk";
...
export default function AndroidDownloadButton() {
  ...
  useEffect(() => {
    ...
    fetch("/app-version.json", { cache: "no-store" })
      ...
      .then(setLatest)
      .catch(() => setLatest({ version: "3.6.0", versionCode: 36, apkUrl: FALLBACK_APK }));
  ...
  return createPortal(
    <a
      ...
      href={href}
      download={isUpdate ? undefined : "Rosario-Market-3.6.apk"}
      ...
      onClick={() => track(isUpdate ? "apk_update" : "apk_download", { version: latest?.version || "3.6.0", source: "navbar" })}
