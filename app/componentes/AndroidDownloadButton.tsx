...
const FALLBACK_APK = "/downloads/Rosario-Market-3.9.4.apk";
...
export default function AndroidDownloadButton() {
  ...
  useEffect(() => {
    ...
    fetch("/app-version.json", { cache: "no-store" })
      ...
      .then(setLatest)
      .catch(() => setLatest({ version: "3.9.4", versionCode: 47, apkUrl: FALLBACK_APK }));
  ...
  return createPortal(
    <a
      ...
      href={href}
      download={isUpdate ? undefined : "Rosario-Market-3.9.4.apk"}
      ...
      onClick={() => track(isUpdate ? "apk_update" : "apk_download", { version: latest?.version || "3.9.4", source: "navbar" })}
