...
const FALLBACK_APK = "/downloads/Rosario-Market-3.8.apk";
...
export default function AndroidDownloadButton() {
  ...
  useEffect(() => {
    ...
    fetch("/app-version.json", { cache: "no-store" })
      ...
      .then(setLatest)
      .catch(() => setLatest({ version: "3.8.0", versionCode: 42, apkUrl: FALLBACK_APK }));
  ...
  return createPortal(
    <a
      ...
      href={href}
      download={isUpdate ? undefined : "Rosario-Market-3.8.apk"}
      ...
      onClick={() => track(isUpdate ? "apk_update" : "apk_download", { version: latest?.version || "3.8.0", source: "navbar" })}
