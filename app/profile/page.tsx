...
interface RecentConv {
  ...
  lastMessage?: { text?: string; image?: string; createdAt: string } | null;
  unreadCount: number;
  updatedAt: string;
}
...
async function saveLocationBackend(lat: number, lng: number) {
  try {
        ...
        return (
          <button
            ...
            style={{
              ...
              borderRadius: 20,
              border: `1.5px solid ${active ? "#f97316" : "#111"}`,
...
function ProfileBannerModal({
  ...
  const total = activeAnns.length + 1; // +1 para el aviso de seguridad

  const [idx, setIdx] = useState(() => {
    const firstUnseen = activeAnns.findIndex(
      (a) => !seenAnnouncements.has(a._id),
    );
  ...
  const cdText = useCountdown(
    current?.expiresAt || new Date(Date.now() + 99999999).toISOString(),
  );

  useEffect(() => {
    if (current && !seenAnnouncements.has(current._id)) {
      onSeenAnnouncement(current._id);
    }
  }, [idx, current]);
    ...
    >
      <div
        ...
        style={{
          ...
          maxWidth: 420,
          overflow: "hidden",
          animation: "profileBannerSlideIn 0.3s cubic-bezier(0.34,1.56,0.64,1)",
        }}
      >
        {/* Header strip */}
        <div
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span
              style={{ color: "#fff", fontWeight: 800, fontSize: "0.88rem" }}
            >
        ...
        <div style={{ padding: "18px 20px" }}>
          {isSecurity ? (
            <>
              ...
              >
                Para tu tranquilidad, acordá el pago y la entrega por el chat.
            ...
            <>
              ...
              {current.link && (
                ...
                >
                  <ExternalLink size={13} />
                  {current.link.length > 40
                    ? current.link.slice(0, 40) + "…"
                    : current.link}
                </a>
              )}
              <div
...
// ── ProfileBannerTrigger ──────────────────────────────────────────────────────
function ProfileBannerTrigger({
  hasUnseen,
  glowActive,
  onClick,
}: {
    ...
    <button
      ...
      style={{
        ...
        boxShadow: glowActive
          ? "0 0 0 4px rgba(249,115,22,0.2), 0 0 16px rgba(249,115,22,0.25)"
          : "none",
        animation: glowActive
          ? "profileGlowPulse 1.8s ease-in-out infinite"
          : "none",
        flexShrink: 0,
      }}
    >
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/user/profile`, {
      ...
      .then((r) => {
        ...
        if (!ct.includes("application/json")) throw new Error("HTML");
        return r.json();
      })
      .then((data) => {
  ...
  const loadAnnouncements = useCallback(async () => {
    ...
    try {
      ...
      // Cargar cuáles fueron vistos
      const seen = new Set<string>();
      active.forEach((a) => {
        if (localStorage.getItem(`profile_ann_seen_${a._id}`)) seen.add(a._id);
      });
      setSeenAnnouncements(seen);

      // Glow si hay algo nuevo
      const securitySeen = !!localStorage.getItem(SECURITY_SEEN_KEY);
      const hasUnseenAnn = active.some(
        (a) => !localStorage.getItem(`profile_ann_seen_${a._id}`),
      );
  ...
  // Reportes sobre mi contenido (vendedor)
  const loadContentReports = useCallback(async () => {
    const token = getToken();
    if (!token) return;
    setContentReportsLoading(true);
    try {
      const res = await fetch(`${API}/reports/on-my-content`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
        const data = await res.json();
      ...
      setContentReportsLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    if (!user) return;
    loadMyReports();
    if (user.role === "seller") loadContentReports();
  }, [user?.id]);
  ...
  useEffect(() => {
    ...
    fetch(`${API}/user/following-businesses`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((r) => (r.ok ? r.json() : []))
  ...
  // Cargar cercanos al montar si ya tiene ubicación
  useEffect(() => {
    if (!user) return;
    const u = user as any;
    if (u.locationEnabled && u.lat && u.lng) {
  ...
  const handleRadiusChange = useCallback(
    (newRadius: number) => {
      ...
      const u = user as any;
      if (u?.locationEnabled && u?.lat && u?.lng) {
        fetchNearby(u.lat, u.lng, newRadius);
      }
    },
    [user],
  );
    ...
    async (lat: number, lng: number, radius: number) => {
      ...
      try {
        ...
        const res = await fetch(
          `${API}/business/nearby?lat=${lat}&lng=${lng}&radius=${effectiveRadius}`,
        );
        if (!res.ok) throw new Error();
        const data: NearbyBusiness[] = await res.json();
        setNearbyBiz(data);
      } catch {
        setNearbyError("No se pudieron cargar los negocios cercanos.");
      } finally {
  ...
  useEffect(() => {
    ...
    // ── Reportes ──────────────────────────────────────────────────────────
    socket.on("report_action_taken", (data: any) => {
      setReportNotifs((prev) => [
        {
          id: Date.now().toString(),
          message: data.message,
          type: data.isDismissed ? "info" : "success",
        },
        ...prev.slice(0, 4),
      ]);
      loadMyReports();
    ...
    socket.on("report_received", (data: any) => {
      setReportNotifs((prev) => [
        {
          id: Date.now().toString(),
          message: data.message,
      ...
      if ((user as any)?.role === "seller") loadContentReports();
    });

    socket.on("report_resolved", (data: any) => {
      setReportNotifs((prev) => [
        {
          id: Date.now().toString(),
          message: data.message,
    ...
    socket.on("product_moderated", (data: any) => {
      setReportNotifs((prev) => [
        {
          id: Date.now().toString(),
          message: data.message,
    ...
    socket.on("product_deleted_admin", (data: any) => {
      setReportNotifs((prev) => [
        {
          id: Date.now().toString(),
          message: data.message,
    ...
    // ── Anuncio nuevo en tiempo real ───────────────────────────────────────
    socket.on("new_announcement", (ann: Announcement) => {
      setAnnouncements((prev) =>
        prev.some((a) => a._id === ann._id) ? prev : [ann, ...prev],
      );
      setGlowActive(true);
      setTimeout(() => setGlowActive(false), 8000);
    });

    // ── Notificación de suscripción (solo vendedores) ──────────────────────
    socket.on(
      "subscription_expiring",
      (data: { message: string; daysLeft: number }) => {
        setReportNotifs((prev) => [
          {
            id: Date.now().toString(),
            message: data.message,
  ...
  const u = user as any;
  const locationActive = !!u.locationEnabled;

  // ── Derived announcement state ────────────────────────────────────────────
  const nowAnn = new Date();
  const activeAnns = announcements.filter(
    (a) => new Date(a.expiresAt) > nowAnn,
  );
  ...
  const handleLocationToggle = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        await saveLocationBackend(lat, lng);
        updateUser({ locationEnabled: true, lat, lng } as any);
        await fetchNearby(lat, lng, selectedRadius);
        setLocationLoading(false);
        const radiusLabel =
          RADIUS_OPTIONS.find((o) => o.value === selectedRadius)?.label ||
          "3 km";
        Swal.fire({
          icon: "success",
          title: "¡Ubicación activada!",
          text: `Mostrando negocios en ${radiusLabel}`,
          timer: 2000,
          showConfirmButton: false,
        });
      },
      () => {
        ...
        Swal.fire({
          ...
          html: "Revisá los permisos del navegador.<br>Hacé click en 🔒 → <b>Ubicación</b> → <b>Permitir</b>",
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    if (newPwd.length < 6) {
      Swal.fire({ icon: "warning", title: "Mínimo 6 caracteres" });
      return;
    }
    setPwdSaving(true);
    try {
  ...
  const handleNotifToggle = async () => {
    ...
    if (!("Notification" in window)) {
      Swal.fire({ icon: "error", title: "No soportado" });
      return;
    }
    if (Notification.permission === "denied") {
      Swal.fire({
        ...
        html: "Hacé click en el candado 🔒 → Notificaciones → Permitir",
        confirmButtonColor: "#f97316",
      });
      return;
    }
  ...
  const handleUnfollow = async (bizId: string, bizName: string) => {
    const Swal = (await import("sweetalert2")).default;
    const { isConfirmed } = await Swal.fire({
      title: `¿Dejar de seguir a ${bizName}?`,
      icon: "question",
      showCancelButton: true,
      confirmButtonText: "Sí, dejar",
      cancelButtonText: "Cancelar",
      confirmButtonColor: "#ef4444",
    });
    if (!isConfirmed) return;
    try {
      const res = await fetch(`${API}/business/${bizId}/unfollow`, {
        method: "POST",
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      if (res.ok) {
        setFollowedBiz((prev) => prev.filter((b) => b._id !== bizId));
        Swal.fire({
          icon: "info",
  ...
  const radiusLabel =
    RADIUS_OPTIONS.find((o) => o.value === selectedRadius)?.label || "3 km";
  const pendingMyReports = myReports.filter(
    (r) => r.status === "pending",
  ).length;
  const pendingContentReports = contentReports.filter(
    (r) => r.status === "pending",
  ).length;
  ...
  return (
    <MainLayout>
      ...
      <style>{`
        ...
        @keyframes profileBannerSlideIn {
      ...
      <div className="profile-page">
        {/* ── Toasts de reportes / suscripción ── */}
          ...
          >
            {reportNotifs.map((n) => (
              ...
              >
                ...
                <button
                  onClick={() =>
                    setReportNotifs((prev) => prev.filter((x) => x.id !== n.id))
                  }
                  style={{
                    background: "none",
                    border: "none",
        ...
        {showBanner && totalUnread > 0 && (
          <div className="profile-chat-banner">
            <div className="profile-chat-banner-left">
              <div className="profile-chat-banner-badge">
                {totalUnread > 99 ? "99+" : totalUnread}
              </div>
              <div>
                <p className="profile-chat-banner-title">
                  💬{" "}
                  {totalUnread === 1
                    ? "Tenés 1 mensaje nuevo"
                    : `Tenés ${totalUnread} mensajes sin leer`}
                </p>
                <p className="profile-chat-banner-sub">
            ...
            <div className="profile-chat-banner-actions">
              <button
                className="profile-chat-banner-btn"
                onClick={() =>
                  router.push(
                    latestConvId
              ...
              <button
                className="profile-chat-banner-close"
        ...
        <div className="profile-header">
          ...
          <div className="profile-info">
            ...
            <div className="profile-email">
              <Mail size={13} strokeWidth={1.75} /> {user.email}
            </div>
            <span className="profile-role">
              {user.role === "seller" ? (
                ...
                <>
                  <LayoutGrid size={12} strokeWidth={1.75} /> Administrador
                </>
              ) : (
                <>
            ...
            {/* ── Botón de anuncios/seguridad — visible y accesible ── */}
          ...
          <div className="profile-stats">
            ...
            <div
              className="profile-stat profile-stat--chat"
        ...
        <div className="profile-body">
          ...
          <div className="profile-sidebar">
            <div className="profile-card">
              ...
              <div className="profile-setting">
                <div className="profile-setting-info">
                  <h4>
                    <Bell size={13} strokeWidth={1.75} /> Notificaciones
                  </h4>
              ...
              <div className="profile-setting">
                ...
                <label
                  className="toggle"
                  style={{ opacity: locationLoading ? 0.5 : 1 }}
                >
                  <input
                    type="checkbox"
              ...
              {u.lat && (
                <div className="profile-coords" style={{ marginTop: "0.5rem" }}>
                  ...
                  <button
                    onClick={() => fetchNearby(u.lat, u.lng, selectedRadius)}
                    title="Actualizar negocios cercanos"
                    style={{
                      background: "none",
                      border: "none",
                      cursor: "pointer",
            ...
            <div className="profile-card">
              ...
              <div className="profile-quick-links">
                <Link
                  href="/mis-productos"
                  className="btn btn-outline profile-quick-btn"
                >
                  <Package size={15} strokeWidth={1.75} /> Mis productos
                </Link>
                {u.businessId && (
                  <Link
                    href={`/negocio?id=${u.businessId}`}
                    className="btn btn-outline profile-quick-btn"
                  >
                ...
                <Link
                  href={
                    latestConvId && totalUnread > 0
                      ? `/chatpage?conversationId=${latestConvId}`
                      : "/chatpage"
                  }
                  className="btn profile-quick-btn profile-chat-btn"
                >
                  <MessageCircle size={15} strokeWidth={1.75} />
                  {totalUnread > 0
                    ? `Ver chats (${totalUnread > 99 ? "99+" : totalUnread})`
                    : "Ir a mis chats"}
                  {totalUnread > 0 ? (
                    <span className="profile-chat-badge">
                      {totalUnread > 99 ? "99+" : totalUnread}
                    </span>
                  ) : (
                    <ExternalLink
          ...
          <div className="profile-main">
            ...
            <div className="profile-card">
              <div className="profile-chat-header">
                <h3>
                  ...
                  Negocios cerca tuyo
                  {nearbyBiz.length > 0 && (
                    <span className="profile-chat-badge profile-chat-badge--title">
                      {nearbyBiz.length}
                    </span>
                  )}
                </h3>
                {locationActive && u.lat && (
                  <div
                    style={{
                      display: "flex",
                      alignItems: "center",
                  ...
                  >
                    {/* Radio buttons inline — con fix de colores */}
                    <div
                      style={{
                        display: "flex",
                        gap: "0.25rem",
                      ...
                      {RADIUS_OPTIONS.map((opt) => (
                        <button
                          ...
                          style={{
                            ...
                            borderRadius: 20,
                            border: `1.5px solid ${opt.value === selectedRadius ? "#f97316" : "#111"}`,
                            background:
                              opt.value === selectedRadius ? "#f97316" : "#fff",
                            color:
                              opt.value === selectedRadius ? "#fff" : "#111",
                            fontSize: "0.7rem",
                            fontWeight:
                              opt.value === selectedRadius ? 700 : 500,
                            cursor: nearbyLoading ? "not-allowed" : "pointer",
                            transition: "all 0.15s",
                            whiteSpace: "nowrap",
                            opacity: nearbyLoading ? 0.5 : 1,
                          }}
                        >
                          {opt.label}
                        </button>
              ...
              {!locationActive ? (
                <div className="profile-chat-empty">
                  ...
                  >
                    Descubrí negocios cercanos y elegí el radio que mejor te
                    sirva.
                  </p>
                  ...
                  >
                    ...
                    {locationLoading
                      ? "Obteniendo ubicación…"
                      : `Activar · buscar en ${radiusLabel}`}
                  </button>
                </div>
              ) : nearbyLoading ? (
                <div className="profile-chat-skeletons">
                  {[...Array(3)].map((_, i) => (
                    <div key={i} className="profile-chat-skeleton" />
                  ))}
                </div>
              ) : nearbyError ? (
                <div className="profile-chat-empty">
                  ...
                  {nearbyBiz.map((biz) => (
                    <div key={biz._id} className="profile-followed-biz-item">
                      <Link
                        ...
                        className="profile-followed-biz-info"
                      >
                        <img
                          src={bizLogoUrl(biz.name, biz.logo)}
                          alt={biz.name}
                          className="profile-followed-biz-logo"
                          onError={(e) => {
                            (e.target as HTMLImageElement).src =
                              `https://ui-avatars.com/api/?name=${encodeURIComponent(biz.name)}&background=f97316&color=fff&size=80`;
                          }}
                        />
                        <div className="profile-followed-biz-meta">
                          <div className="profile-followed-biz-name">
                            {biz.name}
                            {biz.verified && (
                              <span
                                style={{
                          ...
                          <div className="profile-followed-biz-sub">
                            ...
                            >
                              <Navigation size={10} /> {biz.distanceLabel}
                            </span>
                            {biz.address && (
                              <span
                                style={{
                            ...
                            {(biz.rating ?? 0) > 0 && (
                              ...
                              >
                                <Star
                                  ...
                                  stroke="#f97316"
                                />
                                {biz.rating?.toFixed(1)} ({biz.totalRatings})
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="profile-followed-biz-actions">
                        <Link
                          href={`/negocio/${biz._id}`}
                          className="profile-followed-biz-btn profile-followed-biz-btn--visit"
                        >
                          Ver tienda
                        </Link>
                        <ReportModal
            ...
            <div className="profile-card">
              <div className="profile-chat-header">
                <h3>
                  ...
                  {pendingMyReports > 0 && (
                    <span
                      className="profile-chat-badge profile-chat-badge--title"
                      style={{ background: "#f59e0b" }}
                    >
                      {pendingMyReports} pendiente
                      {pendingMyReports !== 1 ? "s" : ""}
                    </span>
                  )}
                </h3>
                <button
                ...
                >
                  {myReports.map((r) => (
                    <div
                      ...
                      style={{
                        background:
                          "var(--surface-alt, rgba(255,255,255,0.04))",
                        border: "1px solid rgba(255,255,255,0.08)",
                        borderRadius: 12,
                        padding: "0.75rem 1rem",
                        borderLeft: `3px solid ${r.status === "action_taken" ? "#22c55e" : r.status === "dismissed" ? "#94a3b8" : "#f59e0b"}`,
                      }}
                    >
                      <div
                        style={{
                      ...
                      <div
                        style={{
                          ...
                          marginBottom: r.adminNote ? "0.35rem" : 0,
                        }}
                      >
              ...
              <div className="profile-card">
                <div className="profile-chat-header">
                  ...
                  >
                    <RefreshCw
                      size={12}
                      style={{
                        animation: contentReportsLoading
                          ? "spin 1s linear infinite"
                          : "none",
                      }}
                    />
                ...
                {contentReportsLoading ? (
                  ...
                  <div className="profile-chat-empty">
                    ...
                    />
                    <p style={{ fontSize: "0.85rem" }}>
                      No tenés reportes sobre tu contenido. ¡Todo en orden!
                    </p>
                  </div>
                ) : (
                  <div
                    ...
                    {contentReports.map((r) => (
                      <div
                        ...
                        style={{
                          ...
                          borderRadius: 12,
                          padding: "0.75rem 1rem",
                          borderLeft: `3px solid ${r.status === "action_taken" ? "#ef4444" : r.status === "dismissed" ? "#94a3b8" : "#f59e0b"}`,
                        }}
                      >
                        <div
                          style={{
                        ...
                        {r.detectedKeywords?.length > 0 && (
                          ...
                          >
                            ...
                            {r.detectedKeywords.map((kw: string) => (
                              <span
                                key={kw}
                                style={{
                                  fontSize: "0.62rem",
            ...
            <div className="profile-card profile-chat-card">
              <div className="profile-chat-header">
                <h3>
                  ...
                  {totalUnread > 0 && (
                    <span className="profile-chat-badge profile-chat-badge--title">
                      {totalUnread > 99 ? "99+" : totalUnread} nuevo
                      {totalUnread !== 1 ? "s" : ""}
                    </span>
                  )}
                </h3>
                <Link
                  ...
                  className="profile-chat-see-all"
                >
                  Abrir chat <ExternalLink size={13} />
                </Link>
              </div>
              {convsLoading ? (
                <div className="profile-chat-skeletons">
                ...
                <div className="profile-chat-empty">
                  ...
                  <p style={{ fontSize: "0.8rem", color: "#bbb" }}>
                    Visitá un negocio y contactalos para comenzar.
                  </p>
                </div>
              ) : (
                <div className="profile-chat-list">
                  {recentConvs.map((conv) => {
                    ...
                    return (
                      <Link
                        key={conv._id}
                        href={`/chatpage?conversationId=${conv._id}`}
                        className={`profile-conv-item${conv.unreadCount > 0 ? " has-unread" : ""}`}
                      >
                        <div className="profile-conv-avatar-wrap">
                          <img
                            src={avatarUrl(conv.other)}
                            alt={conv.other?.name || "Usuario"}
                            className="profile-conv-avatar"
                            onError={(e) => {
                              (e.target as HTMLImageElement).src =
                                `https://ui-avatars.com/api/?name=${encodeURIComponent(conv.other?.name || "?")}&background=f97316&color=fff&size=80`;
                            }}
                          />
                          {conv.unreadCount > 0 && (
                            <span className="profile-conv-dot" />
                          )}
                        </div>
                        <div className="profile-conv-info">
                          <div className="profile-conv-name">
                            {conv.other?.name || "Usuario"}
                          </div>
                          <div
                            className={`profile-conv-last${conv.unreadCount > 0 ? " unread" : ""}`}
                          >
                            {lastText || (
                              <span
                        ...
                        <div className="profile-conv-meta">
                          {conv.lastMessage && (
                            <span className="profile-conv-time">
                              <Clock size={10} />{" "}
                              {timeAgo(conv.lastMessage.createdAt)}
                            </span>
                          )}
                          {conv.unreadCount > 0 && (
                            <span className="profile-conv-badge">
                  ...
                  <Link href="/chatpage" className="profile-chat-all-btn">
                    Ver todas las conversaciones <ChevronRight size={15} />
                  </Link>
                </div>
              )}
            </div>
              ...
              {followedBizLoading ? (
                ...
                <div className="profile-chat-empty">
                  <Store
                    ...
                    style={{ color: "#d1d5db" }}
                  />
                  <p>No seguís ninguna tienda aún.</p>
                  <Link
                    href="/"
                  ...
                  {followedBiz.map((biz) => (
                    <div key={biz._id} className="profile-followed-biz-item">
                      ...
                      <div className="profile-followed-biz-actions">
                        ...
                        />
                        <button
                          className="profile-followed-biz-btn profile-followed-biz-btn--unfollow"
                          onClick={() => handleUnfollow(biz._id, biz.name)}
                        >
                          Dejar de seguir
                        </button>
                      </div>
            ...
            <div className="profile-card">
              <h3>
                <User size={15} strokeWidth={1.75} /> Datos personales
              </h3>
            ...
            <div className="profile-card">
              <h3>
                <KeyRound size={15} strokeWidth={1.75} /> Seguridad
              </h3>
              <form className="profile-form" onSubmit={handleChangePassword}>
                ...
                <button
                  type="submit"
                  className="btn btn-outline profile-save-btn"
                  disabled={pwdSaving}
                >
              ...
              >
                <h3 style={{ color: "#ef4444" }}>
                  <AlertTriangle size={15} strokeWidth={1.75} /> Zona de peligro
                </h3>
                <div className="profile-danger-zone">
                  <div className="profile-danger-info">
                    ...
                    <p>
                      ...
                      productos ni conversaciones.
                    </p>
                  </div>
                  <Link href="/eliminausuario" className="profile-danger-btn">
